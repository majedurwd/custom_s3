...
const uploader = ({
	...
	const destination = (req: Request, file: Express.Multer.File, cb: any) => {
		const companyDir = path.join(
			__dirname,
			`../../uploads/${req.user.companyName.replace(" ", "_")}-${req.user.company_id}/${new Date().getFullYear()}/${new Date().getMonth() + 1}`,
		);
	...
	const fileFilter = (
		...
		if (!mimeTypes.includes(file.mimetype as AllowedFileTypes)) {
			return cb(
				new AppError(`Unsupported file type: ${file.mimetype}`, 400),
			);
		}
